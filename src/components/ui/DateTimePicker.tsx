...
export default function DateTimePicker({ value, onChange, placeholder = "انتخاب تاریخ و ساعت", inputStyle }: Props) {
  ...
  // Escape overflow-y:auto on admin-main by switching the popup to position:fixed
  const fixPosition = useCallback(() => {
    requestAnimationFrame(() => {
      if (!containerRef.current) return;
      const wrapper = containerRef.current.querySelector(".rmdp-wrapper") as HTMLElement | null;
      if (!wrapper) return;
      const input = containerRef.current.querySelector("input") as HTMLElement | null;
      if (!input) return;
  ...
  return (
    <div ref={containerRef}>
      <RDatePicker
        ...
        onOpen={() => { setFocused(true); fixPosition(); }}
